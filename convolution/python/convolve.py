...
def convolve(gridFile, fluxFiles, uhFiles, stateFiles, outFile, var_names, options, re=earthRadius):
    ...
    for i, fluxFile in enumerate(fluxFiles):
        ...
        for j, pointDict in stateDict.iteritems():
            ...
            if data[i][:, j].max() > huge and options['debug']:
                print '+++++++++++++++++++++++++++'
                ...
                print '==========================='
                tm.sleep(5)  # BART COMMENT: Why sleep here - is that just so you can see what is going on?

    # Make a empty table to put the flows in
    print start, end, td, end-start, 'days'  # BART COMMENT: Should 'days' be hardwired here?
    ...
    for i in xrange(len(fluxFiles)):
        s = times[i][0]
        e = times[i][-1]  # get the start and end values
        si = find_nearest(outTimes, s)
        ei = find_nearest(outTimes, e) + 1  # get the start and end inds
    ...
    # BART COMMENT: Make a write_netcdf function
    ## Write to NC file ##
...
def read_netcdf(ncFile, vars=[], coords=False, verbose=False):
    ...
    if coords:
        if isinstance(vars, str):
            data[vars] = f.variables[vars][coords]
            attr[vars] = f.variables[vars].__dict__
        else:
            for var in vars:
                data[var] = f.variables[var][coords]
                attr[var] = f.variables[var].__dict__
    else:
        if isinstance(vars, str):
            data[vars] = f.variables[vars][:]
            attr[vars] = f.variables[vars].__dict__
        else:
            for var in vars:
...
def read_state_files(state_files):
    ...
    for i, sf in enumerate(state_files):
        try:
            state_dict[j], attr, gattr = read_netcdf(sf)
            state_dict[j]['outlet_lon'] = gattr['outlet_lon']
            state_dict[j]['outlet_lat'] = gattr['outlet_lat']
...
def make_state_dict(uh_files):
    ...
    for i, uf in enumerate(uh_files):
        ...
        state_dict[i]['state_streamflow'] = np.zeros(len(time))  # BART COMMENT: time is undefined, so not sure how this can run
