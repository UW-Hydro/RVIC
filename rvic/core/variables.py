...
class Point(object):
    ...
    def __str__(self):
        return "Point({0}, lat:{1:3.4f}, lon:{2:3.4f}, y:{3:d}, " "x:{4:d})".format(
            self.name, self.lat, self.lon, self.domy, self.domx
        )
...
class Rvar(object):
    ...
    def __init__(
        ...
        self.glob_atts = NcGlobals(
            title="RVIC restart file",
        ...
        for tracer in RVIC_TRACERS:
            ...
            self.ring[tracer] = np.zeros(
                (self.full_time_length, self.n_outlets,), dtype=np.float64
            )
        # ------------------------------------------------------------ #
    ...
    def _check_domain_file(self, domain_file):
        ...
        if input_file == self.RvicDomainFile:
            ...
            raise ValueError("dom files do not match in parameter and " "domain file")
        ...
        if run_type in ["startup", "restart"]:
            ...
            if run_type == "restart":
                ...
                if timestamp != file_timestamp:
                    log.warning(
                        "restart timestamps do not match (%s, %s",
                        file_timestamp,
                        self.timestamp,
                    )
            ...
            if f.variables["unit_hydrograph_dt"][:] != self.unit_hydrograph_dt:
                raise ValueError(
                    "Timestep in Statefile does not match " "timestep in ParamFile"
                )

            if not np.array_equal(
                f.variables["outlet_decomp_ind"][:], self.outlet_decomp_ind
            ):
                raise ValueError(
                    "outlet_decomp_ind in Statefile does not " "match ParamFile"
                )

            if f.RvicDomainFile != self.RvicDomainFile:
                raise ValueError(
                    "RvicDomainFile in StateFile does not match " "ParamFile"
                )
    ...
    def convolve(self, aggrunin, time_ord):
        ...
        self.timestamp = ord_to_datetime(
            self.time_ord, TIMEUNITS, calendar=self._calendar
        )
    ...
    def get_time_mode(self, cpl_secs_per_step):
        ...
        if (self.unit_hydrograph_dt % cpl_secs_per_step == 0) and (
            ...
            log.error(
                "unit_hydrograph_dt must be a multiple of the " "cpl_secs_per_step"
            )
    ...
    def write_restart(self, current_history_files, history_restart_files):
        ...
        timesteps = f.createVariable(
            "timesteps", NC_DOUBLE, ("timesteps",), **self.ncvaropts
        )
        ...
        # UH timestep
        unit_hydrograph_dt = f.createVariable(
            "unit_hydrograph_dt", NC_DOUBLE, (), **self.ncvaropts
        )
        ...
        timemgr_rst_type = f.createVariable(
            "timemgr_rst_type", NC_DOUBLE, (), **self.ncvaropts
        )
        ...
        timemgr_rst_step_sec = f.createVariable(
            "timemgr_rst_step_sec", NC_DOUBLE, (), **self.ncvaropts
        )
        ...
        timemgr_rst_start_ymd = f.createVariable(
            "timemgr_rst_start_ymd", NC_DOUBLE, (), **self.ncvaropts
        )
        timemgr_rst_start_ymd[:] = (
            self._start_date.year * 10000
            + self._start_date.month * 100
        ...
        timemgr_rst_start_tod = f.createVariable(
            "timemgr_rst_start_tod", NC_DOUBLE, (), **self.ncvaropts
        )
        ...
        timemgr_rst_ref_ymd = f.createVariable(
            "timemgr_rst_ref_ymd", NC_DOUBLE, (), **self.ncvaropts
        )
        ...
        timemgr_rst_ref_tod = f.createVariable(
            "timemgr_rst_ref_tod", NC_DOUBLE, (), **self.ncvaropts
        )
        ...
        timemgr_rst_curr_ymd = f.createVariable(
            "timemgr_rst_curr_ymd", NC_DOUBLE, (), **self.ncvaropts
        )
        timemgr_rst_curr_ymd[:] = (
            self.timestamp.year * 10000
            + self.timestamp.month * 100
            + self.timestamp.day
        )
        ...
        timemgr_rst_curr_tod = f.createVariable(
            "timemgr_rst_curr_tod", NC_DOUBLE, (), **self.ncvaropts
        )
        ...
        locfnh.comment = (
            "This variable is NOT needed for startup or branch " "simulations"
        )

        locfnhr = f.createVariable("locfnhr", NC_CHAR, coords, **self.ncvaropts)
        ...
        locfnhr.comment = (
            "This variable NOT needed for startup or branch " "simulations"
        )
        # ------------------------------------------------------------ #
        ...
        # ------------------------------------------------------------ #
        # Write Fields
        oyi = f.createVariable("outlet_y_ind", NC_INT, coords[0], **self.ncvaropts)
        oyi[:] = self.outlet_y_ind
        for key, val in iteritems(share.outlet_y_ind):
            if val:
                setattr(oyi, key, val)

        oxi = f.createVariable("outlet_x_ind", NC_INT, coords[0], **self.ncvaropts)
        oxi[:] = self.outlet_x_ind
        for key, val in iteritems(share.outlet_x_ind):
            if val:
                setattr(oxi, key, val)

        odi = f.createVariable("outlet_decomp_ind", NC_INT, coords[0], **self.ncvaropts)
        ...
        for tracer in RVIC_TRACERS:
            ring = f.createVariable(
                "{0}_ring".format(tracer), NC_DOUBLE, tcoords, **self.ncvaropts
            )
