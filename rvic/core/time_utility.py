...
class Dtime(object):
    ...
    def advance_timestep(self):
        ...
        self.timestamp = ord_to_datetime(
            self.time_ord, TIMEUNITS, calendar=self.calendar
        )
        self.timesteps += 1
    ...
    def __stop(self):
        ...
        if self.stop_option in ["nsteps", "nstep"]:
            ...
            temp = ord_to_datetime(
                self.time_ord + self.dt, TIMEUNITS, calendar=self.calendar
            )
            ...
            raise ValueError("unknown stop_option " "{0}".format(self.stop_option))
        return flag
    ...
    def __rest(self):
        ...
        if self.rest_option in ["nsteps", "nstep"]:
            ...
            raise ValueError("unknown rest_option " "{0}".format(self.rest_option))
        return flag
