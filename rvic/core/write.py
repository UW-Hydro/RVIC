...
def write_agg_netcdf(
    ...
    unit_hydrograph_dt = f.createVariable("unit_hydrograph_dt", NC_INT, (), **ncvaropts)
    ...
    timesteps = f.createVariable("timesteps", NC_INT, ("timesteps",), **ncvaropts)
    ...
    fraction = f.createVariable("fraction", NC_DOUBLE, ("lat", "lon",), **ncvaropts)
    for key, val in iteritems(share.fraction):
        if val:
            setattr(fraction, key, val.encode())

    unit_hydrographs = f.createVariable(
        "unit_hydrograph",
        NC_DOUBLE,
        ("timesteps", "lat", "lon",),
