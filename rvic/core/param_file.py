...
def finish_params(outlets, dom_data, config_dict, directories):
    ...
    if options["SUBSET_DAYS"] and options["SUBSET_DAYS"] < routing["BASIN_FLOWDAYS"]:
        subset_length = int(
            options["SUBSET_DAYS"] * SECSPERDAY / routing["OUTPUT_INTERVAL"]
        )
        outlets, full_time_length, before, after = subset(
            outlets, subset_length=subset_length
        )

        slc = slice(min(len(before), 1000))

        log.debug(
            "plotting unit hydrograph timeseries now for before" " / after subseting"
        )

        title = "UHS before subset"
        pfname = plots.uhs(before[slc], title, options["CASEID"], directories["plots"])
        log.info("%s Plot:  %s", title, pfname)

        title = "UHS after subset"
        pfname = plots.uhs(after[slc], title, options["CASEID"], directories["plots"])
        log.info("%s Plot:  %s", title, pfname)
    else:
        log.info(
            "Not subsetting because either SUBSET_DAYS is null or "
            "SUBSET_DAYS<BASIN_FLOWDAYS"
        )
        for key, outlet in iteritems(outlets):
            outlet.offset = np.zeros(outlet.unit_hydrograph.shape[1], dtype=np.int32)
    ...
    if options["CONSTRAIN_FRACTIONS"]:
        ...
        adjust = False
    outlets, plot_dict = adjust_fractions(
        outlets, dom_data[domain["FRACTION_VAR"]], adjust=adjust
    )
    # ---------------------------------------------------------------- #
    ...
    for key, outlet in iteritems(outlets):
        ...
        outlet.upstream_area = np.sum(
            dom_data[dom_area][outlet.y_source, outlet.x_source]
    ...
    if outlet_y_ind.ndim == 0 or outlet_x_ind.ndim == 0:
        for source, outlet in enumerate(source2outlet_ind):
            unit_hydrograph[:, source] *= area[
                source_y_ind[source], source_x_ind[source]
            ]
        ...
        for source, outlet in enumerate(source2outlet_ind):
            ...
            unit_hydrograph[:, source] /= area[
                outlet_y_ind[outlet], outlet_x_ind[outlet]
            ]
            unit_hydrograph[:, source] *= frac_sources[source]
    # ---------------------------------------------------------------- #
    ...
    for title, data in iteritems(plot_dict):
        pfname = plots.fractions(
            data, dom_x, dom_y, title, options["CASEID"], directories["plots"]
        )
        log.info("%s Plot: %s", title, pfname)
    # ---------------------------------------------------------------- #
    ...
    param_file = os.path.join(
        directories["params"],
        "{0}.rvic.prm.{1}.{2}."
        "nc".format(options["CASEID"], options["GRIDID"], today),
    )
    ...
    param_file_name = os.path.split(config_dict["POUR_POINTS"]["FILE_NAME"])[1]
    glob_atts = NcGlobals(
...
def adjust_fractions(outlets, dom_fractions, adjust=True):
    ...
    plot_dict = {
        "Domain Fractions": dom_fractions,
...
def subset(outlets, subset_length=None):
    ...
    for i, (key, outlet) in enumerate(iteritems(outlets)):
        if i == 0:
            ...
            before = np.append(before, outlet.unit_hydrograph, axis=1)

        outlet.offset = np.empty(outlet.unit_hydrograph.shape[1], dtype=np.int32)
        out_uh = np.zeros(
            (subset_length, outlet.unit_hydrograph.shape[1]), dtype=np.float64
        )
        ...
        for j in pyrange(outlet.unit_hydrograph.shape[1]):
            ...
            if right > full_time_length:
                ...
                log.warning(
                    "Subset centered on UH max extends beyond length "
                    "of unit hydrograph."
                )
                log.warning("--> Outlet %s", outlet)
                log.warning("----> Max Index is %s", maxind)
                log.warning(
                    "----> Last value in subset " "is %s", outlet.unit_hydrograph[-1, j]
                )
                if maxind == full_time_length:
                    log.warning(
                        "maxind == full_time_length, not able to "
                        "resolve unithydrograph"
                    )
            if left < 0 or right > full_time_length:
                raise ValueError(
                    "Subsetting failed left:{0} or right {1} does"
...
def group(outlets, subset_length):
    ...
    gd["unit_hydrograph"] = np.empty((subset_length, n_sources), dtype=np.float64)
    ...
    for i, (key, outlet) in enumerate(iteritems(outlets)):
        ...
        log.debug(
            "%s unit_hydrograph.shape %s", outlet.name, outlet.unit_hydrograph.shape
        )
        # -------------------------------------------------------- #
