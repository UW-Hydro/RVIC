...
def read_station_file(file_name, dom_data, config_dict):
    ...
    while True:
        ...
        uhs2_file = os.path.join(config_dict["UHS_FILES"]["ROUT_DIR"], name + ".uh_s2")
        if active == "1":
...
def read_uhs_files(outlets, dom_data, config_dict):
    ...
    if config_dict["UHS_FILES"]["ROUT_PROGRAM"] == "C":
        for cell_id, outlet in iteritems(outlets):
            ...
            outlets[cell_id].source_decomp_ind = dom_data["cell_ids"][
                outlets[cell_id].y_source, outlets[cell_id].x_source
            ]
            f.close()

            outlets[cell_id].cell_id_source = dom_data["cell_ids"][
                outlets[cell_id].y_source, outlets[cell_id].x_source
            ]
...
def move_domain(dom_data, new_dom_data, outlets):
    ...
    if dom_data["cord_lons"].ndim == 1:
        new_y = np.zeros(len(dom_data["cord_lats"]), dtype=np.int16) - FILLVALUE_I
        new_x = np.zeros(len(dom_data["cord_lons"]), dtype=np.int16) - FILLVALUE_I
    else:
        raise ValueError(
            "Grids must be regular and coordinate variables must \
    ...
    for cell_id, outlet in iteritems(outlets):
        ...
        outlets[cell_id].outlet_decomp_ind = new_dom_data["cell_ids"][
            outlets[cell_id].domy, outlets[cell_id].domx
        ]
        outlets[cell_id].source_decomp_ind = new_dom_data["cell_ids"][
            outlets[cell_id].y_source, outlets[cell_id].x_source
        ]
    # ---------------------------------------------------------------- #
