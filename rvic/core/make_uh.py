...
def rout(pour_point, uh_box, fdr_data, fdr_atts, rout_dict):
    ...
    pour_point.basiny, pour_point.basinx = latlon2yx(
        plats=pour_point.lat,
    ...
    if (
        "VIC" in fdr_atts[rout_dict["FLOW_DIRECTION_VAR"]]
        or fdr_data[rout_dict["FLOW_DIRECTION_VAR"]].max() < 10
    ):
...
def search_catchment(to_y, to_x, pour_point, basin_ids, basin_id):
    ...
    *** Does not handle wrapped coordinates. ***
    ...
    for yy, xx in pyzip(byinds, bxinds):
        if in_catch[yy, xx] >= 0:
            ...
            while True:
                ...
                if (
                    (yy == pour_point.basiny) and (xx == pour_point.basinx)
                ) or in_catch[yy, xx] == 1:
    ...
    log.debug("Expected at most %i upstream grid cells from present station", bsize)
...
# -------------------------------------------------------------------- #
# Make uh river
def make_grid_uh_river(
    t_uh, t_cell, uh, to_y, to_x, pour_point, y_inds, x_inds, count_ds
):
    """
    ...
    for (y, x, d) in pyzip(y_inds, x_inds, count_ds):
        if d > 0:
            ...
            raise ValueError(
                "Got negative value ({0}) for count_ds at y={1}" "x={2}".format(d, y, x)
            )
...
# -------------------------------------------------------------------- #
# Make grid uh
def make_grid_uh(t_uh, t_cell, uh_river, uh_box, to_y, to_x, y_inds, x_inds, count_ds):
    """
    ...
    for (y, x, d) in pyzip(y_inds, x_inds, count_ds):
        ...
        if d > 0:
            ...
            unit_hydrograph[:, y, x] = irf_temp[:t_uh] / np.sum(irf_temp[:t_uh])
        else:
...
# -------------------------------------------------------------------- #
# Adjust the timestep
def adjust_uh_timestep(
    unit_hydrograph, t_uh, input_interval, output_interval, x_inds, y_inds
):
    """
    ...
    if output_interval == input_interval:
        log.debug(
            "No need to aggregate in time (output_interval = "
        ...
        uh_out = np.zeros(
            (t_uh_out, unit_hydrograph.shape[1], unit_hydrograph.shape[2]),
            dtype=np.float64,
        )
        for (y, x) in pyzip(y_inds, x_inds):
            for t in pyrange(t_uh_out):
                uh_out[t, y, x] = unit_hydrograph[t * fac : t * fac + fac, y, x].sum()
    else:
        log.debug(
            "Interpolating unit hydrograph from input_interval: %i to "
            "output_interval: %i",
            input_interval,
            output_interval,
        )
