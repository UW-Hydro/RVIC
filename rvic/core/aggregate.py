...
def make_agg_pairs(pour_points, dom_data, fdr_data, config_dict):
    ...
    outlets = OrderedDict(
        sorted(list(iteritems(outlets)), key=lambda t: t[1].upstream_area, reverse=True)
    )
    # ---------------------------------------------------------------- #
...
def aggregate(in_data, agg_data, res=0, pad=0, maskandnorm=False):
    ...
    # ---------------------------------------------------------------- #
    # Place data
    fraction[ilat_min_ind:ilat_max_ind, ilon_min_ind:ilon_max_ind] += in_data[
        "fraction"
    ]
    unit_hydrograph[:, ilat_min_ind:ilat_max_ind, ilon_min_ind:ilon_max_ind] += in_data[
        "unit_hydrograph"
    ]

    if agg_data:
        fraction[alat_min_ind:alat_max_ind, alon_min_ind:alon_max_ind] += agg_data[
            "fraction"
        ]
        unit_hydrograph[
            :, alat_min_ind:alat_max_ind, alon_min_ind:alon_max_ind
