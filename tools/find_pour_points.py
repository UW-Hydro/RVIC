...
def main():
    ...
    if which_points == "all":
        ...
        basin, x_outlet, y_outlet, max_area, min_x, min_y, max_x, max_y = find_all(
            basin_id, source_area, land_mask, lons, lats, res
        )
    else:
        print("Returning all basin outlet grid cells as pour points")
        basin, x_outlet, y_outlet, max_area, min_x, min_y, max_x, max_y = find_outlets(
            basin_id, source_area, lons, lats, res, verbose
        )
    # ---------------------------------------------------------------- #
    ...
    if os.path.splitext(output_file)[1] != ".nc":
        write_ascii_file(
            basin, x_outlet, y_outlet, max_area, min_x, min_y, max_x, max_y, output_file
        )
    else:
        write_netcdf_file(
            basin, x_outlet, y_outlet, min_x, min_y, max_x, max_y, output_file
        )
    # ---------------------------------------------------------------- #
...
# -------------------------------------------------------------------- #
# write output netcdf
def write_netcdf_file(basin, x_outlet, y_outlet, min_x, min_y, max_x, max_y, out_file):
    """
...
def write_ascii_file(
    basin, x_outlet, y_outlet, max_area, min_x, min_y, max_x, max_y, out_file
):
    """
    save the list of pour points as a comma seperated text file
    This file is directly importable into arcgis for validation purposes
    ...
    out = np.column_stack(
        (basin, x_outlet, y_outlet, max_area, min_x, min_y, max_x, max_y)
    )
    header = (
        "OID, longitude, latitude, basin_area, min_lon, min_lat, " "max_lon, max_lat\n"
    )
...
def process_command_line():
    ...
    parser.add_argument(
        "output_file", metavar="outfile.<txt|nc>", help="Output file (suffix dependent)"
    )
    parser.add_argument(
