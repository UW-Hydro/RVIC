...
def test_convolution_wraper():
    ...
    assert ring.sum() == 4
    np.testing.assert_almost_equal(ring.sum(axis=1), unit_hydrograph.sum(axis=1))
    np.testing.assert_almost_equal(ring.sum(axis=0), [2, 2])
...
def test_convolution_wraper_1_outlet():
    ...
    assert ring.sum() == 4
    np.testing.assert_almost_equal(ring.sum(axis=1), unit_hydrograph.sum(axis=1))
    np.testing.assert_almost_equal(ring.sum(axis=0), [4])
...
def test_convolution_wraper_1_source_1_outlet():
    ...
    source_time_offset = np.zeros(n_sources, dtype=np.int32)
    unit_hydrograph = np.array([[0.0, 0.25, 0.5, 0.25, 0.0, 0.0]]).T
    aggrunin = np.ones((ysize, xsize), dtype=np.float64)
    ring = np.zeros((time_length, n_outlets))
    ...
    assert ring.sum() == 1
    np.testing.assert_almost_equal(ring.sum(axis=1), unit_hydrograph.sum(axis=1))
    np.testing.assert_almost_equal(ring.sum(axis=0), [1])
