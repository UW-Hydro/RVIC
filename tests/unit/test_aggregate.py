...
config_dict = {}
config_dict["DOMAIN"] = {"LONGITUDE_VAR": "LONGITUDE", "LATITUDE_VAR": "LATITUDE"}
config_dict["ROUTING"] = {
...
def dom_data(scope="function"):
    dom = {}
    lons, lats = np.meshgrid(np.linspace(-10, 0.0, 20), np.linspace(40, 50, 20))
...
def fdr_data(scope="function"):
    ...
    lons, lats = np.meshgrid(
        np.arange(-15, 15, fdr["resolution"]), np.arange(20, 60, fdr["resolution"])
    )
...
def in_data(fdr_data, scope="function"):
    ...
    rout_data["unit_hydrograph_dt"] = 3600.0
    return rout_data
...
def test_make_agg_pairs_all_unique(dom_data, fdr_data):
    pour_points = pd.DataFrame(
        {"lons": [-2.3, 0.3, 10.4, 17.8], "lats": [42.1, 40.5, 49.0, 45.2]}
    )
...
def test_aggregate_no_agg_data(in_data, fdr_data):
    agg_data = aggregate(
        in_data, {}, res=fdr_data["resolution"], pad=0, maskandnorm=False
    )
...
def test_aggregate_with_aggdata(in_data, agg_data, fdr_data):
    agg_data = aggregate(
        in_data, agg_data, res=fdr_data["resolution"], pad=0, maskandnorm=False
    )
    assert all([k in agg_data for k in in_data])
